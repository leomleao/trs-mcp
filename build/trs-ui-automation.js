...
async function isAuthenticatedPortalPage(page, baseUrl) {
    const url = page.url();
...
async function clickAzureLoginButton(page) {
...
async function selectFirstAzureAccount(page) {
...
async function handleAzureStaySignedIn(page) {
...
async function attemptAzureAutoLogin(page, baseUrl) {
...
async function handleLoginIfNeeded(page, baseUrl) {
...
function getInteractionRoots(page) {
    return [page, ...page.frames()];
}
async function getVisibleCount(root, selector) {
...
async function findTimeRecordingRoot(page) {
...
async function ensureOnEditTimePage(page, baseUrl) {
...
async function openAddTimeModal(dayContainer, root, page) {
    const dateToken = compactDate(await dayContainer
...
async function waitForAddTimeModal(page) {
    const deadline = Date.now() + 15_000;
    while (Date.now() < deadline) {
...
async function chooseFavouriteOption(selectControl, ticketId, description, resolvedTicketCode, resolvedTitle) {
    ...
    async function commitSelection(selectedValue) {
    ...
    const currentValue = await selectControl.inputValue().catch(() => "");
    if (currentValue) {
        const currentLabelText = (await selectControl.locator(`option[value="${currentValue}"]`).first().textContent().catch(() => "")) ?? "";
        const currentLabel = currentLabelText.trim().toLowerCase();
        if (rankedKeywords.some((keyword) => currentLabel.includes(keyword)) ||
...
async function clickAutocompleteSuggestion(page, ticketId) {
...
async function selectTicketViaSearch(page, modal, ticketCode) {
    const searchToggle = (await findFirstVisibleLocator(page, [
        "#radSch",
        "input[type='radio'][id='radSch']",
        "input[type='radio'][value='radSch']",
        "label[for='radSch']",
    ])) ??
        modal.locator("#radSch, input[type='radio'][value*='Sch'], input[type='radio'][id*='radSch'], label[for='radSch']").first();
    ...
    if (!searchInput) {
        return {
            success: false,
            error: formatUiError("ticket_picker_not_found", `Search input not found after toggling search radio. Tried selectors: #txt_tr_sch, input[name='txt_tr_sch'], input[id*='txt_tr_sch'], input[name*='txt_tr_sch']. Page URL: ${page.url()}`),
        };
    }
...
async function selectTicket(modal, page, ticketId, description, bookingMode) {
    ...
    const favouritesToggle = (await findFirstVisibleLocator(page, [
        "#radFav",
        "input[type='radio'][id='radFav']",
        "input[type='radio'][value='radFav']",
        "label[for='radFav']",
    ])) ??
        modal.locator("#radFav, input[type='radio'][value*='Fav'], input[type='radio'][id*='radFav'], label[for='radFav']").first();
    ...
    return {
        success: false,
        error: formatUiError("ticket_picker_not_found", `The favourites dropdown did not retain the selected ticket for '${resolvedTicketCode || description}'.`),
    };
}
async function fillTinyMceComment(page, modal, text) {
    let fillAttempted = false;
    const iframeLocator = (await findFirstVisibleLocator(page, [
        "#txt_tr_comments_ifr",
...
async function findDurationInput(page, modal) {
...
async function extractVisibleValidationText(modal) {
    const validationCandidates = modal.locator(".validation-summary-errors, .field-validation-error, .error, .alert, .message, [role='alert']");
...
async function extractPageMessages(root) {
    const messageCandidates = root.locator(".validation-summary-errors, .field-validation-error, .error, .alert, .message, .success, [role='alert']");
...
async function waitForModalToClose(modal, page) {
    const deadline = Date.now() + 15_000;
    while (Date.now() < deadline) {
...
async function countEntryRows(page, entry) {
...
async function submitAddTimeModal(page, modal) {
    ...
    if ((await submitButton.count()) === 0) {
        return {
            success: false,
            error: formatUiError("submit_failed", `Submit button not found. Tried selectors: #openTime_Add, button#openTime_Add, button:has-text('Add Time'), input[value='Add Time'], input[type='submit'][value='Add Time']. Page URL: ${page.url()}`),
        };
    }
...
async function withRetry(attempts, fn, shouldRetry) {
    let last;
...
async function addEntryToPage(page, entry, debug, retries) {
    try {
        const root = await findTimeRecordingRoot(page);
        ...
        const modal = await withRetry(retries, async () => {
        ...
        if (!modal) {
            ...
            return {
                success: false,
                screenshotPath,
                error: formatUiError("add_time_modal_not_found", `Could not open the Add Time modal after ${retries} attempt(s). Tried ${(await datedButton.count()) > 0 ? "dated button then " : ""}Add Time button candidates. Page URL: ${page.url()}`),
            };
        }
        // Select ticket with retry
        const ticketResult = await withRetry(retries, () => selectTicket(modal, page, entry.ticketId, entry.description, entry.bookingMode), (result) => !result.success);
        ...
        if (!modalClosed) {
            ...
            return {
                success: false,
                screenshotPath,
                error: formatUiError("submit_failed", (validationText ?? "The Add Time modal did not close after submit.") + ` | Page URL: ${page.url()}`),
            };
        }
...
async function fillDatePicker(page, isoDate) {
    ...
    await dateInput.evaluate((el, value) => {
        ...
        if (jq) {
            try {
                jq(input).datepicker("setDate", value);
            }
            catch { /* ignore */ }
        }
        input.value = value;
...
async function submitAndKeepOpen(page, modal) {
    const addAnotherButton = (await findFirstVisibleLocator(page, [
        "#openTime_AddAn",
...
async function addEntriesBatch(page, entries, debug, retries) {
    const results = [];
    // Open the modal from any available "Add Time" button
    const root = await findTimeRecordingRoot(page);
    const modal = await withRetry(retries, () => openAddTimeModal(root.locator("body").first(), root, page), (result) => result === null);
    if (!modal) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        // Select ticket with retry
        const ticketResult = await withRetry(retries, () => selectTicket(modal, page, entry.ticketId, entry.description, entry.bookingMode), (r) => !r.success);
        if (!ticketResult.success) {
...
async function findTicketFrame(page) {
    try {
    ...
    for (const frame of page.frames()) {
        if ((await frame.locator("#tabGeneral").count().catch(() => 0)) > 0)
            return frame;
    }
    return null;
}
async function extractLinksTabData(ticketFrame) {
    ...
    return ticketFrame.evaluate(() => {
        ...
        if (webLinksTable) {
            for (const row of webLinksTable.querySelectorAll("tbody tr")) {
                ...
                if (cells.length < 2)
                    continue;
                const linkText = cells[0].innerText.trim();
                const viewAnchor = cells[1]?.querySelector("a[href]");
                if (!viewAnchor)
                    continue;
                ...
                if (linkText && url)
                    webLinks.push({ text: linkText, url });
            }
        }
        return { linkedIds: [...seen], webLinks };
    });
}
async function extractSingleTicketContext(page, ticketId, baseUrl, visited) {
    ...
    const general = await ticketFrame.evaluate(() => {
        ...
        const detailsEl = document.querySelector("#txt_ed_details");
    ...
    const comments = await ticketFrame.evaluate(() => {
        ...
        return [...container.querySelectorAll("fieldset.mnu_box_page")].map((fs) => {
            ...
            let commentBy = "";
            let date = null;
            let commentContext = "Customer facing";
            if (legendMatch) {
    ...
    const time = await ticketFrame.evaluate(() => {
        function spanNum(id) {
            const el = document.querySelector(`#${id}`);
        ...
        if (tbody) {
            for (const row of tbody.querySelectorAll("tr")) {
                ...
                entries.push({
                    ...
                    durationCON: Number(cells[2].innerText.trim()) || 0,
                    durationCUS: Number(cells[3].innerText.trim()) || 0,
                    approved: cells[4]?.innerText?.trim() !== "\u00a0" &&
...
async function findWorklistRoot(page) {
...
export async function getMyWorklistViaUi(options = {}) {
...
export async function getTicketContextViaUi(ticketId, options = {}) {
    const baseUrl = options.baseUrl ?? DEFAULT_BASE_URL;
    const { page } = await getSharedBrowser(options);
    try {
...
export async function addTimeEntriesViaUi(entries, options = {}) {
    const baseUrl = options.baseUrl ?? DEFAULT_BASE_URL;
