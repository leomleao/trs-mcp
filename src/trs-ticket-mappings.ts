...
export type BookingMode = "favourite" | "search";
...
export interface CustomTicketAlias {
  alias: string;
  ticketCode: string;
  title?: string;
  bookingMode: BookingMode;
...
export interface LearnedAliasResult {
  created: boolean;
  alias: string;
  ticketCode: string;
  bookingMode: BookingMode;
...
interface CustomAliasStore {
  aliases: CustomTicketAlias[];
}

const CUSTOM_ALIAS_FILE = path.join(process.cwd(), "trs-ticket-synonyms.json");

const SEED_MAPPINGS: TicketMapping[] = [
  {
...
function extractTicketCode(value: string): string | null {
  const match = value.toUpperCase().match(/\b[A-Z]{3,}-\d+\b/);
  return match ? match[0] : null;
}

async function readCustomAliasStore(): Promise<CustomAliasStore> {
  try {
    const file = await fs.readFile(CUSTOM_ALIAS_FILE, "utf8");
    const parsed = JSON.parse(file) as Partial<CustomAliasStore>;
    return {
...
async function writeCustomAliasStore(store: CustomAliasStore): Promise<void> {
  await fs.writeFile(CUSTOM_ALIAS_FILE, JSON.stringify(store, null, 2), "utf8");
}

export async function isKnownAlias(alias: string): Promise<boolean> {
  const normalizedAlias = normalize(alias);
  if (!normalizedAlias) {
    return false;
  }

  const seedKnown = SEED_MAPPINGS.some((mapping) =>
    mapping.aliases.some((entry) => normalize(entry) === normalizedAlias) || normalize(mapping.title) === normalizedAlias,
  );
  if (seedKnown) {
    return true;
  }
...
export async function listTicketMappings(): Promise<{ seeds: TicketMapping[]; customAliases: CustomTicketAlias[] }> {
  const store = await readCustomAliasStore();
  return { seeds: SEED_MAPPINGS, customAliases: store.aliases };
}

export async function upsertCustomTicketAlias(alias: CustomTicketAlias): Promise<CustomTicketAlias> {
  const normalizedAlias = normalize(alias.alias);
  const normalizedTicketCode = alias.ticketCode.toUpperCase();
  const store = await readCustomAliasStore();

  const existingIndex = store.aliases.findIndex(
    (entry) => normalize(entry.alias) === normalizedAlias || entry.ticketCode.toUpperCase() === normalizedTicketCode,
  );

  const nextAlias: CustomTicketAlias = {
...
function scoreMapping(phrase: string, mapping: { aliases: string[]; title?: string; ticketCode: string }): { score: number; matchedAlias?: string } {
...
export async function resolveBookingSelection(ticketId: string, description: string): Promise<BookingResolution | null> {
  ...
  const seedMatches = SEED_MAPPINGS.map((mapping) => ({ mapping, ...scoreMapping(phrase, mapping) })).sort(
